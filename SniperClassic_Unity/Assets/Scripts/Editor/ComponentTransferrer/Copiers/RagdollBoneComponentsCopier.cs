...
namespace HenryTools.Editor
{
    ...
    public class RagdollBoneComponentsCopier : UnityComponentCopier
    {
        ...
        protected override void HandlePastedComponent(GameObject selected, List<Component> remainingComponents)
        {
            ...
            for (int i = 0; i < storedUnityComponents.Count; i++)
            {
                ...
                System.Type componentType = storedUnityComponents[i].GetType();

                if (componentType == typeof(CharacterJoint))
                {
                    HandlePasteJoint((CharacterJoint)storedUnityComponents[i], selected.GetComponent<CharacterJoint>());
                }
